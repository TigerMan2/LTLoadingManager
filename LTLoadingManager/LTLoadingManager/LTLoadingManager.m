...
/** 加载的提示语 */
static NSString         *LTLoadingMessage = @"loading...";

/** 加载成功的提示语 */
static NSString         *LTSuccessMessage = @"加载成功";
/** 加载成功的图片 */
static NSString         *LTSuccessImageName = @"loading...";

/** 加载失败的提示语 */
static NSString         *LTFailMessage = @"加载失败";
/** 加载失败时的图片 */
static NSString         *LTFailImageName = @"loading...";

/** 默认简短语的显示时间 */
static NSTimeInterval   LTShowTime = 1.5f;

@implementation LTLoadingManager
...
+ (void)showLoadingHudWithInView:(UIView *)view isShowMaskView:(BOOL)isShowMaskView {
    [self showLoadingHudWithMessage:LTLoadingMessage inView:view isShowMaskView:isShowMaskView];
}
...
+ (void)showLoadingHudWithMessage:(NSString *)message inView:(UIView *)view isShowMaskView:(BOOL)isShowMaskView completionBlock:(MBProgressHUDCompletionBlock)completionBlock {
    ...
    hud.label.text = message ? message : LTLoadingMessage;
    hud.completionBlock = completionBlock;
}

#pragma mark - 显示状态

+ (void)showStateHudWithMessage:(NSString *)message state:(LTHUDShowType)state {
    [self showStateHudWithMessage:message state:state imageName:nil];
}

+ (void)showStateHudWithMessage:(NSString *)message state:(LTHUDShowType)state imageName:(NSString *)imageName {
    [self showStateHudWithMessage:message state:state imageName:imageName isShowMaskView:NO];
}

+ (void)showStateHudWithMessage:(NSString *)message state:(LTHUDShowType)state imageName:(NSString *)imageName isShowMaskView:(BOOL)isShowMaskView {
    [self showStateHudWithMessage:message state:state imageName:imageName isShowMaskView:isShowMaskView afterDelay:0];
}

+ (void)showStateHudWithMessage:(NSString *)message state:(LTHUDShowType)state imageName:(NSString *)imageName isShowMaskView:(BOOL)isShowMaskView afterDelay:(NSTimeInterval)delay {
...
+ (void)showStateHudWithMessage:(NSString *)message state:(LTHUDShowType)state imageName:(NSString *)imageName isShowMaskView:(BOOL)isShowMaskView afterDelay:(NSTimeInterval)delay inView:(UIView *)view {
...
+ (void)showStateHudWithMessage:(NSString *)message state:(LTHUDShowType)state imageName:(NSString *)imageName isShowMaskView:(BOOL)isShowMaskView afterDelay:(NSTimeInterval)delay inView:(UIView *)view completionBlock:(MBProgressHUDCompletionBlock)completionBlock {
    NSString *titleMessage = nil;
    NSString *imageString = nil;
    switch (state) {
        case LTHUDShowTypeSuccess:
        {
            titleMessage = message ? message : LTSuccessMessage;
            imageString  = imageName ? imageName : LTSuccessImageName;
        }
            break;
        case LTHUDShowTypeFail:
        {
            titleMessage = message ? message : LTFailMessage;
            imageString  = imageName ? imageName : LTFailImageName;
        }
            break;
...
+ (void)showMessage:(NSString *)message {
    [self showMessage:message afterDelay:LTShowTime];
}

+ (void)showMessage:(NSString *)message inView:(UIView *)view {
    [self showMessage:message afterDelay:LTShowTime inView:view];
}
...
+ (void)showHudWithCustomView:(UIView *)customView isShowMaskView:(BOOL)isShowMaskView inView:(UIView *)view {
    [self showHudWithCustomView:customView isShowMaskView:isShowMaskView afterDelay:LTShowTime inView:view];
}
...
+ (void)showHudWithMessage:(NSString *)message customImageView:(UIView *)customImageView mode:(MBProgressHUDMode)mode isShowMaskView:(BOOL)isShowMaskView afterdelay:(NSTimeInterval)delay inView:(UIView *)view completionBlock:(MBProgressHUDCompletionBlock)completionBlock {
    ...
    //隐藏hud
    NSTimeInterval showTime = delay>0 ? delay : LTShowTime;
